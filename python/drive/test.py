...
for url in urls:
    ...
    # 데이터 추출
    items = soup.select("#modalStoreInfo > div")

    for item in items:
        detail_button = item.select_one("#container > div.content > div.contArea > div > div > div.mcStore > table > tbody > tr")
        print("찾기")
        if detail_button:
            # 요소를 찾습니다.
            ...
            if element:
                ...
                track = soup.select("#modalStoreInfo")

                title_element = track.select_one("#modalStoreInfo > div > div.storeDetailInfo > h2")
                title = title_element.text.strip() if title_element else "No Title"
                
                address_element = track.select_one("#modalStoreInfo > div > div.storeDetailInfo > table > tbody > tr > td")
                address = address_element.text.strip() if address_element else "No Address"

                image_element = track.select_one("#modalStoreInfo > div > div.storeImgWr > ul > li.on > a > img")
                ...
                # 모달 닫기
                close_button = browser.find_element(By.CSS_SELECTOR, "#modalStoreInfo > div > button")
                close_button.click()
                time.sleep(1)
